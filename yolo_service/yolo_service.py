...
except Exception:  # optional dependency; only required when using Azure storage
...
FRAME_STORE = os.getenv("FRAME_STORE", "local").strip().lower()  # local | azure
AZURE_CONTAINER = os.getenv("AZURE_CONTAINER", "imageframes").strip()
AZURE_PREFIX = os.getenv("AZURE_PREFIX", "frames").strip().strip("/")

if FRAME_STORE == "local":
...
def _get_blob_container_client():
    """
    Returns an Azure Blob ContainerClient when FRAME_STORE=azure.
    ...
    if BlobServiceClient is None:
        raise RuntimeError(
            "FRAME_STORE=azure but azure-storage-blob is not installed. "
    ...
    if conn:
        ...
        acct = os.getenv("AZURE_STORAGE_ACCOUNT", "").strip()
        key = os.getenv("AZURE_STORAGE_KEY", "").strip()
        if not acct or not key:
            raise RuntimeError(
                "FRAME_STORE=azure requires AZURE_STORAGE_CONNECTION_STRING "
    ...
    try:
        ...
        pass  # already exists / no permission; okay if it exists

    _blob_container_client = container
    return _blob_container_client


def _blob_name(kind: str, idx: int, ts: str) -> str:
    # kind: "raw" | "annotated"
    if kind == "raw":
        filename = f"{ts}_frame_{idx:06d}.png"
        return f"{AZURE_PREFIX}/raw/{filename}"
    if kind == "annotated":
        filename = f"{ts}_frame_{idx:06d}_annotated.png"
        return f"{AZURE_PREFIX}/annotated/{filename}"
...
def save_raw(pil_img, idx, ts):
    """
    Saves raw PNG either locally (captured_frames/) or to Azure Blob Storage.
    Returns a string identifier for the saved object.
    """
    if FRAME_STORE == "azure":
        container = _get_blob_container_client()
        blob_name = _blob_name("raw", idx, ts)
        buf = BytesIO()
        ...
        container.upload_blob(
            ...
            data=buf,
            overwrite=True,
            content_settings=ContentSettings(content_type="image/png") if ContentSettings else None,
        )
        return f"azure://{AZURE_CONTAINER}/{blob_name}"
...
def save_annotated(b64, idx, ts):
    ...
    if FRAME_STORE == "azure":
        container = _get_blob_container_client()
        blob_name = _blob_name("annotated", idx, ts)
    ...
    cv2.imwrite(
        os.path.join(ANNOTATED_DIR, f"{ts}_frame_{idx:06d}_annotated.png"),
        bgr,
    )
...
def list_frames():
    ...
    return jsonify({"storage": "local", "count": len(files), "files": files})
...
def clear_frames():
    ...
    if FRAME_STORE == "azure":
        container = _get_blob_container_client()
        prefixes = [f"{AZURE_PREFIX}/raw/", f"{AZURE_PREFIX}/annotated/"]
        for pref in prefixes:
            blob_names = [b.name for b in container.list_blobs(name_starts_with=pref)]
...
if __name__ == "__main__":
    ...
    if FRAME_STORE == "azure":
        log.info(f"  Frames → azure://{AZURE_CONTAINER}/{AZURE_PREFIX}/")
    else:
